...
class TCPRelayHandler(object):
    ...
    def _update_activity(self, data_len=0):
        ...
        self._data_len_to_relay += data_len
    ...
    def _handle_stage_connecting(self, data):
        ...
        self._data_to_write_to_remote.append(data)
        if (self._is_local or self._is_relay) and not self._fastopen_connected and \
        ...
        if result:
            ...
            if ip:
                ...
                try:
                    ...
                    if (self._is_local or self._is_relay) and self._config['fast_open']:
                        # for fastopen:
    ...
    def _on_local_read(self):
        ...
        if not self._is_local and not self._is_relay:
            data = self._encryptor.decrypt(data)
            if not data:
                return
        if self._stage == STAGE_STREAM:
            ...
            return
        elif self._is_local and not self._is_relay and self._stage == STAGE_INIT:
        ...
        elif (self._is_local and not self._is_relay and self._stage == STAGE_ADDR) or \
                ((not self._is_local or self._is_relay) and self._stage == STAGE_INIT):
    ...
    def destroy(self):
        ...
        if self._remote_address:
            logging.info('destroy: %s:%d, data_len: %d, local: %s:%d, listen: 0.0.0.0:%d, server: %s:%d' %
                          (self._remote_address[0]
                         , self._remote_address[1]
                         , self._data_len_to_relay
                         , self._client_address[0]
                         , self._client_address[1]
                         , self._server._listen_port
                         , self._chosen_server[0]
                         , self._chosen_server[1]))
        else:
            logging.info('destroy, na:na, data_len: %d, local: %s:%d, listen: 0.0.0.0:%d, server: %s:%d' % 
                         (self._data_len_to_relay
                        , self._client_address[0]
                        , self._client_address[1]
                        , self._server._listen_port
